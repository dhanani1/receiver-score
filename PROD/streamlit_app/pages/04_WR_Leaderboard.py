# /04_WR_Leaderboard.py (updated)
import streamlit as st
import plotly.graph_objects as go
from utils import load_weekly, add_weekly_rates, aggregate_and_rate
import pandas as pd

st.set_page_config(page_title="WR Leaderboard", layout="wide")
st.title("WR Leaderboard")
...
mask = (df["Season"].eq(season) & df["Seas_Type"].eq(seas_type) &
        df["Week"].isin(weeks) & df["db_pos"].astype(str).eq("WR"))
...
if view == "Aggregated":
    ...
    cols = [c for c in [
        "Player_Name","Team","receiver_tier","receiver_score","routes_week","targets_week",
    ...
    # Weekly RR columns
    if "receiver_score_per_route" in t.columns:
        t["receiver_score_rr_week"] = t["receiver_score_per_route"]
    if "receiver_score_tier_per_route" in t.columns:
        t["receiver_tier_rr_week"] = t["receiver_score_tier_per_route"]
    cols = [c for c in [
        "Season","Seas_Type","Week","Player_Name","Team","receiver_tier_rr_week","receiver_score_rr_week",
...
DISPLAY_MAP = {
    "Player_Name":"Player","receiver_tier":"Receiver Tier / RR","receiver_score":"Receiver Score / RR",
    "receiver_tier_rr_week":"Receiver Tier / RR","receiver_score_rr_week":"Receiver Score / RR",
...
cfg = {"Receiver Score / RR": st.column_config.NumberColumn(format="%.0f"),
...
# Scatter: X=Routes, Y=Receiver Score / RR
if view == "Aggregated" and not df_show.empty and {"Receiver Score / RR","Routes"}.issubset(df_show.columns):
    fig = go.Figure()
    fig.add_trace(go.Scatter(
        x=df_show["Routes"], y=df_show["Receiver Score / RR"], mode="markers",
        hovertemplate="%{text}<br>Routes %{x:.0f} | RR Score %{y:.0f}<extra></extra>",
    ...
    fig.update_layout(title="Routes vs Receiver Score / RR",
                      xaxis=dict(title="Routes", tickformat="d"),
                      yaxis=dict(title="Receiver Score / RR", range=[0,100], tickformat="d"),
